...
ipcMain.on('win:close', async e => {
  const w = BrowserWindow.fromWebContents(e.sender);

  // Demander mot de passe
  const { response, checkboxChecked } = await dialog.showMessageBox(w, {
    type: 'question',
    buttons: ['Quitter', 'Annuler'],
    defaultId: 1,
    cancelId: 1,
    title: 'Quitter NexOS',
    message: 'Entrez le mot de passe pour quitter :',
    detail: 'Mot de passe requis pour fermer NexOS',
    noLink: true,
  });

  if (response === 0) {
    const { response: pass } = await dialog.showInputBox({ title: 'Mot de passe', label: 'Mot de passe :' });
    if (pass === EXIT_PASSWORD) w.close();
    else dialog.showErrorBox('Erreur', 'Mot de passe incorrect !');
  }
});
